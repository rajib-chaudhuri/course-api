...
node {
	...
	stage('Code quality analysis') {
		...
		withSonarQubeEnv('dxp sonarqube server') {
			...
			//sh "'${mvnHome}/bin/mvn' -Dmaven.test.failure.ignore  clean package sonar:sonar"
			sh "'${mvnHome}/bin/mvn' test  clean package sonar:sonar"
	...
	stage('DockerImage run'){
		...
		sh "bash  /var/lib/jenkins/workspace/dxpcommerce@libs/dxp-pipeline-library/vars/runContainer.sh ${applicationName} ${publishPort} ${applicationPort}"
